...
import type { SafeguardListParams } from '../api/safeguard'
...
export const useSafeguardStore = defineStore('safeguards', () => {
  ...
  async function load(params: SafeguardListParams = {}) { loading.value = true; try { items.value = (await safeguardApi.listSafeguards(params)).items; if (!items.value.some((x) => x.id === selectedId.value)) selectedId.value = items.value[0]?.id } finally { loading.value = false } }
